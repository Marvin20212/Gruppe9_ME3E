...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="docGlobalName" select ="'./DBDIGlobalRepConversion.htm'"/>
    <!-- HTML -->
    <xsl:variable name="docProcessReportName" select ="'../../ProcessReport.html'"/>
    <!-- HTML -->

    <html>
      <head>
        <link rel="stylesheet" type="text/css" href="..\Data\ReportFBDI.css"/>
        <script id="scripttag" type="text/javascript" src="..\Data\ReportFBDI_HTML.js"></script>
        <!-- HTML -->
      </head>
      <body bgcolor="White">
            ...
            <tbody  id="title">
              ...
              <tr>
                <td align="center" colspan="2">
                    ...
                    <xsl:when test ="$docTag  &gt; 2 and $packNum>1">
                      <A>
                        <xsl:attribute name ="href">
                          <xsl:value-of select ="concat($docRepName,$packNum - 1,'.htm')"/>
                          <!-- HTML -->
                        </xsl:attribute>
                  ...
                  <xsl:choose>
                    <xsl:when test ="$docTag  &gt; 2 and ($packNum or $docTag = 4 or $docTag = 7 )">
                      <A>
                        <xsl:attribute name ="href">
                  ...
                  <xsl:choose>
                    <xsl:when test ="$docTag  &gt; 2 and $packNum &lt; $nbOfPack">
                      <A>
                        <xsl:attribute name ="href">
                          <xsl:value-of select ="concat($docRepName,$packNum + 1,'.htm')"/>
                          <!-- HTML -->
                  ...
                  <th class="titleMode"  colspan="5" onmouseover="this.className='highlightTitleMode clickable';" onmouseout="this.className='titleMode clickable';">
                    ...
                    <xsl:choose>
                      <xsl:when test  = "starts-with($docType, 'FBDI_Split')">
                        Scope Completion : documents and package attribution
                      </xsl:when>
                      <xsl:otherwise>
      ...
      <tr >
        <td align = "left" >
          <xsl:if test="/ReportInfo/ReportInformation/Details/Detail/text() != ''">
            <b><xsl:value-of select = "ReportInfo/ReportVariable[@Name='ReportName']/@Value"/></b> : <xsl:value-of select="/ReportInfo/ReportInformation/Details/Detail[last()]"/>
          </xsl:if>
                      ...
                      <xsl:when test ="starts-with($paramDocType, 'FBDI_Split')">
                        <xsl:choose>
                          <xsl:when test ="$ScopeStatus = 'Error'">
                            <xsl:attribute name ="src">../Data/SB_Result_Err_expand.png</xsl:attribute>
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:attribute name ="src">../Data/SB_Result_expand.png</xsl:attribute>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:when>
                      <xsl:when test ="$ScopeStatus = 'Error'">
                        <xsl:attribute name ="src">../Data/SB_Step_Err_collapse.png</xsl:attribute>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:attribute name ="src">../Data/SB_Step_expand.png</xsl:attribute>
                      </xsl:otherwise>
                    </xsl:choose>
                  ...
                  <img>
                    <xsl:choose>
                      ...
                      <xsl:when test ="$ScopeStatus = 'Warning'">
                        <xsl:attribute name ="src">../Data/SB_Step_Warning_collapse.png</xsl:attribute>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:attribute name ="src">../Data/SB_Step_expand.png</xsl:attribute>
                      </xsl:otherwise>
                      ...
                      <xsl:when test ="starts-with($paramDocType, 'FBDI_Split')">
                        <xsl:choose>
                          <xsl:when test ="$ConversionStatus = 'Error'">
                            <xsl:attribute name ="src">../Data/Split_Err_collapse.png</xsl:attribute>
                          </xsl:when>
                          <xsl:when test ="$ConversionStatus = 'Interrupted' or $ConversionStatus = 'Skipped'">
                            <xsl:attribute name ="src">../Data/Split_NotExe.png</xsl:attribute>
                          </xsl:when>
                          <xsl:otherwise>
                            <xsl:attribute name ="src">../Data/Split_expand.png</xsl:attribute>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:when>
                      <xsl:when test ="$ConversionStatus = 'Error'">
                        <xsl:attribute name ="src">../Data/Convert_Step_Err_collapse.png</xsl:attribute>
                      </xsl:when>
                      <xsl:when test ="$ConversionStatus = 'Interrupted' or $ConversionStatus = 'Skipped'">
                        <xsl:attribute name ="src">../Data/Convert_Step_NotExe.png</xsl:attribute>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:attribute name ="src">../Data/Convert_Step_expand.png</xsl:attribute>
                      </xsl:otherwise>
                    </xsl:choose>
                  ...
                  <img>
                    <xsl:choose>
                      <xsl:when test ="$SaveStatus = 'Error'">
                        <xsl:attribute name ="src">../Data/Save_Step_Err_collapse.png</xsl:attribute>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:choose>
                          <xsl:when test ="$SaveStatus = 'Interrupted' or $SaveStatus = 'Skipped'">
                            <xsl:attribute name ="src">../Data/Save_Step_NotExe.png</xsl:attribute>
                          </xsl:when>
                          <xsl:otherwise>

                            <xsl:attribute name ="src">../Data/Save_Step_expand.png</xsl:attribute>
                          </xsl:otherwise>
                        </xsl:choose>
            ...
            <table align="left" width="100%">
              ...
              <tr>
                ...
                <xsl:if test="starts-with($docType, 'DBDI_Rep') and LinkInformation/TEReportName != 'TransactionReport_0'">
                  ...
                  <A>
                    <xsl:attribute name="href">
                      <xsl:value-of select ="concat('../../',$TEReport,'.html#',$TEObject)"/>
                      <!--HTML-->
                    </xsl:attribute>
            ...
            <xsl:if test ="($strStepType ='ScopeBuild' or not(starts-with($strDocType, 'FBDI_Split')) ) and not($strStepStatus ='OK' and $strStepType !='ScopeBuild') and $strStepStatus != 'Skipped'" >
              <img class="clickable">
                <xsl:attribute name ="src">
                  <xsl:choose>
                    <xsl:when test ="$strStepStatus = 'Error'">..\Data\Minus.gif</xsl:when>
                    <xsl:otherwise>..\Data\Plus.gif</xsl:otherwise>
                  </xsl:choose>
                </xsl:attribute>
                <xsl:attribute name = "id">
  ...
  <xsl:template name ='CreatePointingPointedTable'>
    ...
    <table class ="objectFBDITable" width="100%">
      ...
      <th class ="pointingMode clickable">
        ...
        &#160;
        <xsl:choose>
          <xsl:when test = "$strTableType = 'Pointing'">Points</xsl:when>
          <xsl:otherwise>
                ...
                <table cellspacing="1" cellpadding="3" align="left" class="objectFBDITable"  witdh="100%" >
                  ...
                  <xsl:for-each select="$raisedMesssageToProcess">
                    ...
                    <xsl:variable name ="infraCode">
                      <xsl:choose>
                        <xsl:when test ="@Type = 'IdOfRaisedMsg'">
                          <xsl:value-of select ="//ReportInfo[@Type='RaisedMsg' and @Number = $CurrentIdOfRaisedMsg ]/ReportInformation/Tag/Code"/>
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:value-of select ="./ReportInformation/Tag/Code"/>
                        </xsl:otherwise>
                    ...
                    <xsl:variable name ="source">
                      <xsl:choose>
                        <xsl:when test ="@Type = 'IdOfRaisedMsg'">
                          <xsl:value-of select ="//ReportInfo[@Type='RaisedMsg' and @Number = $CurrentIdOfRaisedMsg ]/ReportInformation/Source	"/>
                        </xsl:when>
                        <xsl:otherwise>
                          <xsl:value-of select ="./ReportInformation/Source"/>
                        </xsl:otherwise>
                    ...
                    <xsl:variable name ="nbOfRaisedMessage">
                      <xsl:value-of select="count(/descendant-or-self::ReportInfo[@Type ='RaisedMsg'][./ReportInformation/Tag/Code = $infraCode])"/>
                    </xsl:variable>
                      ...
                      <td>
                        ...
                        <span class ="displayHide">
                          ...
                          <table align="left" width= "100%">
                            ...
                            <xsl:for-each select="/descendant-or-self::ReportInfo[@Type ='document'][./descendant::ReportInfo[@Type='RaisedMsg']/ReportInformation/Tag/Code = $infraCode]">
                              <xsl:variable name ="nbOfMsgInCurrentDoc" select="count(./descendant::ReportInfo[@Type='RaisedMsg'][./ReportInformation/Tag/Code = $infraCode])"/>
                              <xsl:variable name ="oneRaisedMsgNumber" select="./descendant::ReportInfo[@Type='RaisedMsg'][./ReportInformation/Tag/Code = $infraCode]
												[not(./ReportInformation/Tag/Code = preceding-sibling::ReportInfo[@Type='RaisedMsg']/ReportInformation/Tag/Code)]/@Number"/>
                              ...
                              <xsl:variable name="HasDetail" select="count(./descendant::ReportInfo[@Type='RaisedMsg'][./ReportInformation/Tag/Code = $infraCode][./ReportInformation/Details/Detail])"/>
                              <xsl:variable name ="CurrentSource">
                                <xsl:value-of select ="./descendant::ReportInfo[@Type='RaisedMsg' and @Number = $oneRaisedMsgNumber]/ReportInformation/Source"/>
                              </xsl:variable>
                              <xsl:variable name ="nbOfSameSource">
                                <xsl:value-of select="count(./descendant::ReportInfo[@Type ='RaisedMsg'][./ReportInformation/Source = $CurrentSource])"/>
                              </xsl:variable>
  ...
  <xsl:template name="DetailInfoPackage">
    ...
    <table cellspacing="1" cellpadding="3" align="center" class="objectFBDITable" >
      ...
      <xsl:for-each select="/ReportInfo/Children/ReportInfo[@Type='Package']">
        ...
        <xsl:variable name ="packReportPath" select ="concat('../PackageDir',$packageNumber,'/','HTML','/',$docNameWithReport,$packageNumber,'_Report.htm')"/>
        <!-- HTML -->
